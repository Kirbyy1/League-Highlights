...
- Shows saved clips in the Highlights page and opens them in the default Windows video player.
...
Enable **Create one highlight reel after every game** to produce one chronological `MATCH_HIGHLIGHTS_...mp4` containing only the accepted moments from that match. Overlapping sections are trimmed so the same fight is not repeated. Manual clips can be included, and you can choose whether the original individual clips are kept after a reel succeeds. A reel is created only when at least two unique moments were saved.
